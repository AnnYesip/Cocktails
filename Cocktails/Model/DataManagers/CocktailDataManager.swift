...
final class CocktailDataManager {
      ...
      cocktails.forEach { (drink) in
        ...
        queue.async {
          ...
          DispatchQueue.main.async {
            coreDataManager.saveAlcoholicCocktails(
              drink.strDrink,
              image: data,
              id: drink.idDrink,
  ...
  func downloadNonAlcoholicDataCocktails() {
    coreDataManager.deleteAllData()
    let queue = DispatchQueue(label: "images", qos: .utility)
    self.networkManager.fetchData(for: .nonAlcoholic) { [unowned self] (cocktail) in
      cocktail.forEach { (drink) in
        ...
        queue.async {
          guard let data = try? Data(contentsOf: imageUrl) else { return }
          DispatchQueue.main.async {
            coreDataManager.saveNonAlcoholicCocktails(
              drink.strDrink,
              image: data,
              id: drink.idDrink,
              strInstructions: drink.strInstructions ?? "рецепт отсутствует"
    ...
    self.networkManager.fetchData(for: .random) { [unowned self] (cocktail) in
      cocktail.forEach { (drink) in
        ...
        queue.async {
          guard let data = try? Data(contentsOf: imageUrl) else { return }
          DispatchQueue.main.async {
            coreDataManager.saveRecommendedCocktails(
              drink.strDrink,
              image: data,
              id: drink.idDrink,
              strInstructions: drink.strInstructions ?? "рецепт отсутствует"
    ...
    self.networkManager.fetchDataById(id: id) { cocktail in
      cocktail.forEach { (drink) in
        ...
        queue.async {
          guard let data = try? Data(contentsOf: imageUrl) else { return }
          DispatchQueue.main.async {
            self.coreDataManager.saveSearchByIdCocktail(
              drink.strDrink,
              image: data,
              id: drink.idDrink,
